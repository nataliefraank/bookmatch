...
export async function GET(request: NextRequest) {
  ...
  if (q.length < 2) {
    return NextResponse.json({ books: [] as BrowseBook[] });
  }

  const ol = `https://openlibrary.org/search.json?q=${encodeURIComponent(q)}&limit=40&fields=title,author_name,cover_i`;
  ...
  try {
    ...
    const books = mapDocs(data.docs ?? [], 24);
    return NextResponse.json({ books });
  } catch (e) {
    console.error("GET /api/books/search:", e);
